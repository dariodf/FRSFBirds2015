...
public class TrajectoryPlanner {
	...
	public boolean trajectoriaObstruida(Rectangle sling, ArrayList<Point> release, Point target, ABObject object){
		...
		// Compruebo:
		// Si el chancho esta entre los extremos horizontales del bloque
		// Si el chancho esta entre los extremos verticales

		boolean TargetALaIzquierda = target.x < object.x,
		...
		if(object.y < target.y)
			return false;
				 ...
				 */
				// Si el bloque está horizontalmente mas lejos de la resortera que el target
				// entonces no puede obstruir un tiro directo.
				if(TargetALaIzquierda)
					return false;
				...
				for (Point p : release) {
					...
					if(TargetEntreX){
						if(TargetSobreObjeto && (y >= object.y))
							return true;
						else if( targetDebajoObjeto && (y <= object.y + object.height))
							return true;
					}
					else {
						//				if(	TargetSobreObjeto && (object.y <= y && object.y <= yWidth) )	
						//					return true;
						//				else if(targetDebajoObjeto && 
						//						((object.y > y && object.y <= yWidth) ||
						//						 (object.y + object.height >= y && object.y <= y)))	
						//								return true;
						if ((object.y > y && object.y <= yWidth) ||
								(object.y + object.height >= y && object.y <= y))
							return true;
					}
					//			if ((y >= object.y) || 
