...
/**
*	This class is used to store screenshot from the disk, plot trajectories and objects on them and later store them on the disk.
*/
...
public class DLGraphics
{
	...
	*/
	public DLGraphics(BufferedImage canvas)
	{
		this("img", canvas);
	}
	/**
	*	Creates a canvas from the screenshot that is provided with the given filename.
	*/
	public DLGraphics(String filename, BufferedImage canvas)
	{
	...
	{
		final String imgFilename = String.format(path + File.separator + "%06d_" + _filename + _filenameAppend + ".png", _nOfScreens++);
	...
	*/
	 ...
	 *
	 * @param
	 *			tp - trajectory planner instance
	 *          slingshot - bounding rectangle of the slingshot
	 *          releasePoint - point where the mouse click was released from
	 *			objects - if specified, the trajectory is drawed red where collides with objects
	 * @return  the canvas with trajectory drawn
	 */
	
	public void plotTrajectory(TrajectoryPlanner tp, Rectangle slingshot, Point releasePoint, ABType birdOnSling)
	{
	...
	public void plotTrajectory(TrajectoryPlanner tp, Rectangle slingshot, Point releasePoint, ABType birdOnSling, List<ABObject> objects)
	{
			...
			if ((p.y > 0) && (p.y < _canvas.getHeight(null)))
			{
				...
				if (objects != null)
					for (ABObject ob : objects)
						if (ABUtil.pointHitsObjectWithinARange(ob, p, birdOnSling.getBirdRadius()))
	...
	*/
	...
	{
		drawString(String.format("Trajectory utility: %d, heuristic utility: %d", trajectoryUtility, heuristicUtility), 200, 45, Color.red);			
	}
