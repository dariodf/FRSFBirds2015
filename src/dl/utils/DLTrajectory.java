...
/**
*	DLTrajectory encapsulates the trajectory planning. 
* 	it stores the trajectory points, its utility and the objects that are in the way.
*/

public class DLTrajectory
{
    ...
    public static final int _birdsBlocksDamageTrajectory[][] =
    {
        ...
        // { 500,  NA,1400,  NA,1200,  NA,0,0,0,5500, 700, 500,1400},// RED_BIRD
        // { 450, 600,3100,4200,1300,1400,0,0,0,5400,3400, 600,4200},// YELLOW_B
        // { 100, 300, 300,1200,3300,ALOT,0,0,0,5400, 100,3100,3300},// BLUE_B
        // {3600,ALOT,1450,ALOT,2000,ALOT,0,0,0,5300,3050,3600,1450},// BLACK_B
	...
	*/
	public DLTrajectory(ClientActionRobot tmpActionRobot, TrajectoryPlanner tmpTp, Rectangle tmpSling, ABType tmpBirdOnSling, Point tmpReleasePoint, Point tmpTargetPoint, ABObject tmpTargetObject,List<ABObject> tmpHills, List<ABObject> tmpBlocks, List<ABObject> tmpPigs)
	{
	...
	/**
	*	this function computes the trajectory utility based on the objects that are in the way.
	*/
	...
	*/
	public void estimateHillsAndBlocksInTheWayForWhiteBird()
	{
		...
		while (!ABUtil.pointHitsObjectWithinARange(targetObject, tmpPoint, birdOnSling.getBirdRadius()))
		{		
			...
			tmpPoint = new Point(targetObject.getCenter().x + (int)(Math.round(adjacentSide * xDirection)), targetObject.getCenter().y + (int)(Math.round(oppositeSide * yDirection)));

			eggPoints.add(tmpPoint);
		}		

		// estimate objects in the way
		blocksInTheWay = ABUtil.estimateObjectsInTheWay(eggPoints, blocksAll, targetPoint, targetObject, 1);

		computeBlocksDistanceFromTheTarget(blocksInTheWay);

		// estimate hills in the way
		hillsInTheWay = ABUtil.estimateObjectsInTheWay(eggPoints, hillsAll, targetPoint, targetObject, 1);				
	}
	/**
	...
	*/
	...
	/**
	*	computes the distance from the target object of the particular blocks that are in the way
	*/
	public void computeBlocksDistanceFromTheTarget(List<ABObject> tmpBlocks)
	{
        ...
        if (blocksInTheWay.size() == 0
        	|| trajectoryPoints.size() == 0)
        	return;

        for (ABObject block : blocksInTheWay)
        {
        	// compute distance of the block from the targetPoint minus bird radius, minus targetObject width
        	blocksInTheWayDistanceFromTheTarget.add((block.trajectoryHitPoint.distance(targetObject.getCenter()) - birdOnSling.getBirdRadius()));// - Math.abs(targetObject.width)));
        }
	}
	/**
	...
	*/
	public int getPercentageOfTheTrajectoryWhenTheFirstObjectIsHit(List<ABObject> tmpBlocks)
	{
		...
		return ABUtil.reachablePercentage(sling, blocksInTheWay, targetPoint, targetObject, releasePoint, birdOnSling.getBirdRadius(), trajectoryPoints);
	}
	/**
	...
	*/
	private List<ABObject> estimateObjectsInTheWay(List<ABObject> objects)
	{
		...
		return ABUtil.estimateObjectsInTheWay(trajectoryPoints, objects, targetPoint, targetObject, birdOnSling.getBirdRadius());
	}
	/**
	...
	*/
	...
	{
		List<Point> tmpTrajectory = tp.predictTrajectory(sling, releasePoint, birdOnSling, targetPoint, targetObject, hillsAll, blocksAll);
	...
	*/
	...
	{
		DLGraphics dlg = new DLGraphics(actionRobot.doScreenShot());
	...
	*/
	...
	/**
	*	finds the closest path from an object to a target object.
	* 	this is used in the round stone strategy
	*/
	public List<ABObject> findAPathWithAStar(List<ABObject> objects, ABObject startABObject, ABObject endABObject)
	{
        DLPriorityQueue<ABObject> open = new DLPriorityQueue<ABObject>();
        ...
        while (!open.isEmpty()) 
        {
            ...
            for (ABObject y : current.findAllWhichTouches(objects)) 
            {
                ...
                {
                    // distance either does not include actual key or the new distance is smaller than the original...
